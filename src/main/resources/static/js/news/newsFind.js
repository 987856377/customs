...
function find(){
    ...
    if (index!=null&&index!=""&&value!=null&&value!="") {
        $.post("/news/find",{index:index, value:value},function (response) {
...
$btn_delete.click(function () {
    ...
    swal({
        ...
        if (isConfirm) {
            ...
            $.post("/news/deleteMul", {ids: JSON.stringify(ids)}, function (response) {
                if (response.code == 200) {
...
$btn_update.click(function () {
    ...
    if (id!=null&&id!=""&&title!=null&&title!=""&&content!=null&&content!="") {
        $.post("/news/update",{id:id, title: title, content: content},function (response) {
            if (response.code == 200){
...
window.operateEvents = {
    ...
    'click .remove': function (e, value, row, index) {
        ...
        swal({
            ...
            if (isConfirm) {
                $.post("/news/deleteOne", {id: row.id}, function (response) {
                    if (response.code == 200) {
                        swal("已删除!", "该数据已从您的个人数据中移除!", "success");
                        $table.bootstrapTable('remove', {
                            field: 'id',
